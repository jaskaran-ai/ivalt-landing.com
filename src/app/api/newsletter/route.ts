...
export async function POST(request: NextRequest) {
  try {
    ...
    const adminEmails = process.env.ADMIN_EMAILS || process.env.CONTACT_EMAILS || process.env.CONTACT_EMAIL || 'info@ivalt.com';
    const adminRecipients = adminEmails.split(',').map(email => email.trim()).filter(email => email);
